...
def index():
    ...
    student = (
        db(db.auth_user.username == request.vars.sid)
        .select(
            db.auth_user.id,
    ...
    (
        ...
        auth.user, float(session.timezoneoffset) if "timezoneoffset" in session else 0
    )
    ...
    return dict(
        ...
        practice_message1=message1,
        practice_message2=message2,
    ...
    if (
        ...
        if practice_start_date > now_local.date():
            ...
            practice_completion_count = _get_practice_completion(
                user.id, user.course_name, spacing
            )
...
def practice():
    ...
    if (
        ...
        if practice_completion_today.isempty():
            ...
            practice_completion_count = _get_practice_completion(
                auth.user.id, auth.user.course_name, spacing
            )
            if practice_graded == 1:
