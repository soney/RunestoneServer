...
def grades():
    ...
    studentinfo = {}
    practice_setting = db((db.course_practice.course_name == auth.user.course_name) &
                          (db.course_practice.auth_user_id == auth.user.id)).select().first()
    practice_average = 0
    for s in students:
        practice_grade = 0
        user_practice_condition = db((db.user_practice_condition.course_name == s.course_name) &
                                     (db.user_practice_condition.auth_user_id == s.id)).select().first()
        if user_practice_condition.spacing == 1:
